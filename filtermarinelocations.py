...
if __name__ == '__main__':
    ...
    parser.add_argument('filter_file', type=str, help='path to the filter file (must have a sorted "index" column) (default delimiter: comma)')
    ...
    if args.output_file=='':
        #filename=args.gzip_file.split('.')[0]
        filename = Path(args.gzip_file).stem
        outputfilename = f'{filename}_marine.tsv'
    else:
        outputfilename = args.output_file

    filtersep = args.filter_delimiter
    gzipsep = args.gzip_delimiter
    ...
    with open(args.filter_file, 'r') as filter:
        ...
        # Open the gzip file
        with gzip.open(args.gzip_file, "r") as gbif_data:
            ...
            for idx, line in enumerate(gbif_data):
                ...
                if idx==index: #both `idx` and  `index` start at 0
                    ...
                    if len(obs) != Ncolumns:
                        error+=1
                        print()
                        print(f"    SplittingError: splitting line n°{idx} gives more fields ({len(obs)}) than the header ({nb_columns}).")
