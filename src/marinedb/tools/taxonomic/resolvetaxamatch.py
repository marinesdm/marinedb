...
def apply(inputfile, isinworms_params, review_level=1, taxamatch_key = 'taxamatch_generatedby_isinworms', matchtype_key='classif_matchtype_generatedby_isinworms', auxiliary_columns=None, flag_uncertain=True, manual_filter_file='manual_filter_generatedby_resolvetaxamatch.txt', resume=True, store_manual_filter=True, outputfile=None, verbose=True, indent=''):
    """Interactively review and resolve selected uncertain taxonomic matches.

    Selects potentially false taxonomic mismatches produced by `isinworms`,
    groups them by unique combinations of original taxonomic values and any 
    verbatim values used for authorship matching, and submits these combinations 
    to the interactive matching mode of `isinworms`. The resulting manual 
    decisions are then propagated to all corresponding records in the input file.

    When used through the integrated workflow, the required `isinworms`
    parameters and processing columns are forwarded automatically, so users
    generally need to specify only `review_level`.

    Cases are reviewed in batches of 50 unique taxonomic combinations. Manual
    decisions may be stored in a reusable filter, allowing interrupted reviews
    to be resumed and previously resolved combinations to be applied without
    further user interaction.

    Args:
        inputfile (str):
            Path to the tab-separated file containing the records previously 
            processed by `isinworms`. This value is supplied automatically by 
            the integrated workflow.

        isinworms_params (dict):
            Parameters passed to `isinworms` for interactive taxonomic
            resolution. The dictionary must contain `rank_mapping`. When
            `resolvetaxamatch` is used through the integrated workflow, the
            relevant `isinworms` parameters are forwarded automatically.
            Parameters required specifically for interactive processing are 
            set internally.

        review_level ({1, 2, 3}, optional):
            Level controlling which uncertain matching outcomes are submitted
            for manual review, from those most likely to result from limitations
            of the matching procedure to increasingly reliable mismatches.

            Level `1` reviews cases where higher-rank matching fails despite
            agreement at the scientific-name and kingdom levels
            (`noclassification_kingdomMatch`).

            Level `2` additionally reviews authorship mismatches with consistent
            higher-rank classifications when authority processing appears not to
            have accounted for the complete authority string
            (`classification_authorship_noMatchIsMore`).

            Level `3` further includes cases combining higher-rank and authorship
            mismatches under the same apparent limitation in authority-string
            processing (`noclassification_authorship_noMatchIsMore`).

            Only cases classified as `uncertain` by `isinworms` can be reviewed.
            When this function is used through the integrated workflow, the
            `isinworms` uncertainty level is automatically increased when
            necessary to remain compatible with the requested review level.

        taxamatch_key (str, optional):
            Name of the column containing the matching status generated by
            `isinworms`. This value is supplied automatically by the integrated
            workflow.

        matchtype_key (str, optional):
            Name of the column describing how the `isinworms` decision was
            reached. This value is supplied automatically by the integrated
            workflow.

        auxiliary_columns (list of str, optional):
            Columns required during manual review but not requested in the final
            output. The columns are removed before the output file is written. 
            This list is constructed automatically by the integrated workflow.

        flag_uncertain (bool, optional):
            Whether to retain records that remain `uncertain` after manual
            review. When `False`, these records are removed from the final dataset. This
            value is inherited automatically from the corresponding `isinworms`
            setting by the integrated workflow.

        manual_filter_file (str, optional):
            Path to the tab-separated file used to store and reuse manual
            taxonomic decisions. When only a filename is provided, it is
            resolved relative to the output directory. 

        resume (bool, optional):
            Whether to reuse decisions stored in `manual_filter_file`.
            Previously reviewed taxonomic combinations are excluded from the
            interactive review and their stored decisions are applied
            automatically. If the file is unavailable or incompatible with the
            current input columns, processing restarts from scratch. 

        store_manual_filter (bool, optional):
            Whether to retain the manual-filter file after successful
            processing. The file is still created during interactive review so
            that decisions can be stored incrementally and an interrupted run
            can be resumed.

        outputfile (str, optional):
            Path to the processed tab-separated output file. When omitted or
            empty, a default filename is generated from `inputfile`. If the
            input and output paths are identical, processing is performed
            through a temporary file before the original file is replaced.
    ...
    Raises:
        Exception:
            If the output directory does not contain enough free disk space
        Exception:
            If `isinworms_params` does not contain the required `rank_mapping` 
            key.

    Notes:
        - Review is performed once per unique combination of original
        taxonomic and optional verbatim values used by `isinworms`.
        The resulting decision is propagated to every record sharing that 
        combination.

        - Selecting a WoRMS candidate interactively changes the corresponding
        decision to `match`, whereas rejecting all candidates changes it to
        `nomatch`. Decisions are stored by `isinworms` after each batch of 50
        combinations.

        - Records classified as `nomatch` during manual review are retained and
        flagged when the input file contains a `nomatch` flag column previously 
        generated by `isinworms`. Otherwise, they are removed from the output. 
        Records that remain `uncertain` are retained or removed according to 
        `flag_uncertain`.

        - When called directly with an `isinworms` uncertainty level lower than
        `review_level`, some eligible matching types may already have been
        classified as `nomatch` and cannot be reviewed. Such records are
        excluded with a warning. The integrated workflow prevents this
        incompatibility by adjusting the `isinworms` uncertainty level.

        - After successful processing, the input file is removed when
        `outputfile` refers to a different path. When both paths are identical,
        the original file is replaced by the processed temporary file.
    """
    ...
    if input_is_output:
        root, _ = os.path.splitext(outputfile)
        outputfile = f'{root}.temp'
#        outputfile = outputfile.split('.')[0] + '.temp'

    # Manual filter file path

    if len(os.path.dirname(manual_filter_file)) == 0:
    ...
    if ncombinations != 0:
        ...
        isinworms_params = copy.deepcopy(isinworms_params)
