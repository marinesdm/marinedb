...
def resume_process(filter, values):
    ...
    values_processed = set(filter['group'].tolist())
    values2process = values_processed - set(values)

    return values2process
...
def get_AcceptedClassification(valid_aphiaID):
    ...
    results = _reconnect_getAphiaRecordsByIDs(aphiaID)
