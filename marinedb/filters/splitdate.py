...
def apply(df, key, split_type='all', drop_interval=False, overwrite=False, flag=False):
    ...
    intervalcolumns=set(df.columns) - set(columns)
    ...
    # overwrite=True: overwrite `year`/`month`/`day` if the columns already exist
    # overwrite=False: create new columns to avoid overwriting data in existing columns
