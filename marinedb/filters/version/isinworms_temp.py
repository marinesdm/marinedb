...
def isnan(value):
    ...
    except (ValueError,TypeError): #not an empty or nan-like string
...
def match_TaxaByFullClassification(gbif_classif, worms_classif, allowed_mismatch_withNaN, allowed_mismatch_withoutNaN, keep_fossil=False, spe=None):
    ...
    if any(worms_classif["matchtype_species"]=="match_quarantine"):
        print(spe)
        print(worms_classif)
        print(gbif_classif)
        print()
    ...
    if (any(classif["matchtype_species"]=="match_quarantine")):
...
def apply_matchfilter(classification, matchfilter=None, allowed_mismatch_withNaN=1, allowed_mismatch_withoutNaN=2, outputpath='./', keep_fossil=False):
    ...
    for idx in range(len(classification)):
        ...
        spe = tuple([classification.loc[idx,RANK['species']]])
        if not isnan(classification.loc[idx,RANK['species']]):
            worms_classif = filter.get_group(spe).reset_index(drop=True)
            ...
            classif = match_TaxaByFullClassification(gbif_classif, worms_classif, allowed_mismatch_withNaN, allowed_mismatch_withoutNaN, keep_fossil=keep_fossil,spe=spe)
...
def apply(df, *ignored_args, allowed_mismatch_withNaN=1, allowed_mismatch_withoutNaN=2, matchfilter=None, acceptedfilter=None, outputpath='./', drop_conditions={'matchtype_classif':'nomatch', 'matchtype_species':'match_deleted'},keep_fossil=False):
...
if __name__ == '__main__':
    ...
    df = apply(df, matchfilter=matchfilter, acceptedfilter=acceptedfilter, keep_fossil=True)
