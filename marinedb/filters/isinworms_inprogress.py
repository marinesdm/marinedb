...
def _solve_UncertaintyByAuthorship():
...
def _match_TaxaByFullClassification(gbif_classif, worms_classif, allowed_mismatch_withNaN, allowed_mismatch_withoutNaN, keep_fossil=False):
    ...
    if any(worms_classif["matchtype_species"]=="nomatch"):
...
def apply(df, *ignored_args, allowed_mismatch_withNaN=1, allowed_mismatch_withoutNaN=2, matchfilter=None, acceptedfilter=None, keep_fossil=False, outputpath='./', drop_conditions={'matchtype_classif':'nomatch', 'matchtype_species':'match_deleted'}):
    ...
    if Nobs == 0:
        ...
        df = df.reindex(df.columns.tolist() + ["species", "matchtype_classif"] + new_columns, axis=1)
